...
import { ConfiguracaoNfceService } from "@/lib/services/configuracao-nfce.service"
import { InutilizacaoNfceService } from "@/lib/services/inutilizacao-nfce.service"
import { NfceConfigFields, NfceInutilizacaoFields } from "./nfce-form-tabs"
...
const nfeConfigSchema = z.object({
  ...
  modeloNfce: z.enum(["4.00"]).optional(),
})
...
type NfceFormData = z.infer<typeof nfeSchema>
...
export function NfceForm() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const responseConfig = await ConfiguracaoNfceService.getByEmitente(emitente.id)
        // Buscar inutilizações NFe
        const responseInutilizacao = await InutilizacaoNfceService.getByEmitente(emitente.id)
        ...
        form.reset({
          ...
          modeloNfce: config?.modeloNfce || "4.00",
          // Inutilizações
  ...
  const onSubmitConfig = async () => {
    ...
    try {
      ...
      const payload: any = {
        ...
        // Salvar modelo NFe
        modeloNfce: data.modeloNfce || "4.00",
      }
      ...
      const response = await ConfiguracaoNfceService.upsert(emitenteId, payload)
  ...
  const onSubmitInutilizacao = async () => {
    ...
    try {
      ...
      const response = await InutilizacaoNfceService.upsert(emitenteId, payload)
            ...
            <Tabs value={activeTabConfig} onValueChange={(value) => setActiveTabConfig(value as "producao" | "homologacao")} className="w-full">
              ...
              <TabsContent value="producao" className="mt-4">
                <div className="rounded-md border-l-2 border-l-green-500 bg-green-50/50 dark:bg-green-950/20 p-3">
                  <NfceConfigFields form={form} prefix="Producao" showInutilizacao={false} />
                </div>
              </TabsContent>
              ...
              <TabsContent value="homologacao" className="mt-4">
                <div className="rounded-md border-l-2 border-l-orange-500 bg-orange-50/50 dark:bg-orange-950/20 p-3">
                  <NfceConfigFields form={form} prefix="Homologacao" showInutilizacao={false} />
                </div>
              </TabsContent>
            </Tabs>
              ...
              <TabsContent value="producao" className="mt-4">
                <div className="rounded-md border-l-2 border-l-green-500 bg-green-50/50 dark:bg-green-950/20 p-3">
                  <div className="space-y-3">
                    ...
                    <NfceInutilizacaoFields form={form} prefix="Producao" />
                  </div>
                </div>
              </TabsContent>
              ...
              <TabsContent value="homologacao" className="mt-4">
                <div className="rounded-md border-l-2 border-l-orange-500 bg-orange-50/50 dark:bg-orange-950/20 p-3">
                  <div className="space-y-3">
                    ...
                    <NfceInutilizacaoFields form={form} prefix="Homologacao" />
                  </div>
                </div>
              </TabsContent>
            </Tabs>
