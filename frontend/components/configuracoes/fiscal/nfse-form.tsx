...
const nfeConfigSchema = z.object({
  ...
  modeloNfse: z.enum(["4.00"]).optional(),
})
...
type NfseFormData = z.infer<typeof nfeSchema>
    ...
    async function loadData() {
      try {
        ...
        form.reset({
          ...
          modeloNfse: config?.modeloNfse || "4.00",
          // Inutilizações
  ...
  const onSubmitConfig = async () => {
    ...
    try {
      ...
      const payload: any = {
        ...
        // Salvar modelo NFe
        modeloNfse: data.modeloNfse || "4.00",
      }
      ...
      const response = await ConfiguracaoNfseService.upsert(emitenteId, payload)
